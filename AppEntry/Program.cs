...
class Program {
	public static async Task Main() {
		...
		var db = new Db("Host=127.0.0.2;Port=5433;Database=db3;Search Path=public;Username=postgres;Password=1234");
		...
		if (true) { await AppScenarios.CheckAddBookmarkArticle(db); };
		if (true) { await AppScenarios.CheckRemoveArticleBookmark(db); };
	}
}
